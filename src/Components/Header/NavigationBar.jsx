...
export default function NavigationBar(props) {
    ...
    return(
        <nav className="flex justify-between md:flex-col lg:flex-row border-b-[1px] border-[#979797] px-[2.4rem] md:px-[3.2rem] items-center ">
            ...
            <ul className={`${props.menuIsOpen? "left-[0] " : ""} px-[2.4rem] md:px-0 gap-[20px] md:gap-[33px] uppercase ${style.transition} ${style.navMobile} ${style.navDesktop} flex flex-col md:flex-row md:left-0 md:static fixed left-[-100%] md:translate-x-0 md:flex md:w-fit md:h-fit w-screen bg-primary h-screen text-[1.5rem] md:text-[1.1rem]`}>
                <li onClick={props.setMenu}><Link to={"/"}><h4 className="items-center  flex md:opacity-[0.5] pb-[15px] md:pb-0  border-b md:border-b-0 border-[#979797] md:hover:border-t-2 md:border-lightBlue  transition-all ease-in-out duration-100"><span className="md:hidden inline-block rounded-full w-[2rem] h-[2rem] bg-[#979797] mr-[2.5rem]"></span> mercury</h4></Link></li>
